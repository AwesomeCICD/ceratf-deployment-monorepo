...
module "vault_config" {
  source = "git@github.com:AwesomeCICD/ceratf-module-vault-config?ref=1.12.1"
}
...
module "app_spaces" {
  source           = "git@github.com:AwesomeCICD/ceratf-module-appspaces?ref=3.3.0"
...
module "release_agent" {
  ...
  managed_namespaces = ["default", "guidebook", "boa", "circleci-release-agent-system", "dr-demo", "eddies-demo","training"]
...
module "release_agent_dev" {
  ...
  managed_namespaces = ["guidebook-dev", "boa-dev", "dr-demo-dev","training-dev"]
...
module "authentik" {
  ...
  source        = "git@github.com:AwesomeCICD/ceratf-module-helm-authentik.git?ref=1.0.5"
